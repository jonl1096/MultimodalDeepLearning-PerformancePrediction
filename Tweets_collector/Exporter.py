 ...
 python Exporter.py --username "barackobama" --maxtweets 10 --toptweets\n"""
	...
	try:
		...
		def receiveBuffer(tweets):
			sorted_tweets = []
			for t in tweets:
				sorted_tweets.append((t, t.retweets + t.favorites))
			sorted_tweets.sort(key=lambda tup:tup[1], reverse=True)
			for st in sorted_tweets[:3]:
				t = st[0]
				outputFile.write(('\n%s<;>%s<;>' % (t.date.strftime("%Y-%m-%d"), t.text)))
			outputFile.flush();
		...
		outputFile.close()
		print ('Done. Output file generated "orioles_tweets_%s.csv".' %date[2:4])
